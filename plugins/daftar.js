...
let handler = async function (m, { text, usedPrefix }) {
  ...
  if (!Reg.test(text)) throw `Format salah!\nContoh: *${usedPrefix}daftar PutraGanzz|17*`
  let [_, name, splitter, age] = text.match(Reg)
  let totalreg = Object.keys(global.DATABASE._data.users).length
  if (!name) throw 'Nama tidak boleh kosong!'
  if (!age) throw 'Umur tidak boleh kosong!'
  if (age < 10) throw 'Maaf, Anda belum bisa mendaftar.\n*Minimal umur 10 Ke Atas*'
  if (age > 25) throw 'Maaf, Anda terlalu tua.'
  user.name = name
...
*Bot Ini Database Setiap Pukul 15.00 WIB Akan Direset!!*
`.trim()
